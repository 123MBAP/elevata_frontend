    ...
    <header className="sticky top-0 z-40 bg-white/80 backdrop-blur-md border-b border-gray-200 shadow-sm px-4 py-3 flex items-center justify-between">
      ...
      <div className="flex items-center space-x-3">
        ...
        {user?.role !== 'FINANCIAL_INSTITUTION' ? (
          ...
          <div className="flex items-center space-x-2 px-3 py-1.5 bg-slate-50 border border-slate-200 rounded-lg text-xs font-bold text-slate-700 select-none">
            <Building className="w-3.5 h-3.5 text-slate-500" />
            <span>{user.financialInstitution?.institutionName || 'Credit Institution'}</span>
          </div>
        )}
      </div>
        ...
        <div className="relative">
          ...
          >
            ...
            <div className="text-left hidden md:block">
              <span className="text-[11px] font-bold text-slate-800 block leading-tight font-heading">
                {user?.role === 'ADMIN'
                  ? 'Administrator'
                  : user?.role === 'FINANCIAL_INSTITUTION'
